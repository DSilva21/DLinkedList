...
{
	if (plist->comp == NULL)// 정렬기준이 마련되지 않았으면,
		FInsert(plist, data); //머리에 노드를 추가한다.
	else //정렬기준이 마련되었다면,
		SInsert(plist, data);// 정렬기준에 근거하여 노드를 추가한다.
}
...
int LNext(List *plist, LData *pdata)
{
	...
	plist->before = plist->cur; //cur이 가리킨 것을 before가 가리킴
...
LData LRemove(List*plist)
{
	...
	plist->cur = plist->before; //소멸 대상의 데이터를 rdata에 저장
